    ...
    return (
        <div
            className={ `rounded-lg transition-all border ${isSelected ? 'ring-2 ring-primary bg-block-hover' : 'border-border'
                } cursor-pointer overflow-hidden w-full h-full` }
            style={ cardStyles }
        >
            { block.imageUrl && (
                <img
            ...
            <div className="p-3 sm:p-4 md:p-6 flex flex-col h-full">
                <h3 className="text-base sm:text-lg md:text-xl font-semibold mb-2" style={ textStyles }>
                    { block.title || 'Card Title' }
                </h3>
                <p className="text-sm sm:text-base text-muted-foreground mb-4 flex-grow" style={ textStyles }>
