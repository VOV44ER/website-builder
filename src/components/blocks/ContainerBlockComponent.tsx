...
export const ContainerBlockComponent: React.FC<Props> = ({ block, isSelected, selectedBlockId, onBlockSelect }) => {
    const { reorderBlocksInContainer, deleteBlockFromContainer, duplicateBlock, updateBlock } = useEditor();
    const layoutClass = block.layout === 'horizontal' ? 'flex flex-row' : 'flex flex-col';
    const gap = block.gap || '1rem';

    const handleDragEnd = (result: DropResult) => {
        if (!result.destination) return;
        reorderBlocksInContainer(block.id, result.source.index, result.destination.index);
    };

    const handleDeleteBlock = (blockId: string, e: React.MouseEvent) => {
        e.stopPropagation();
        deleteBlockFromContainer(block.id, blockId);
    };
                        ...
                        >
                            { block.blocks.length === 0 ? (
                                <div className="text-center py-8 text-muted-foreground text-sm w-full">
                                    Контейнер пуст. Добавьте блоки из палитры.
                                </div>
                            ) : (
                                    ...
                                    <Draggable key={ childBlock.id } draggableId={ childBlock.id } index={ index }>
                                        { (provided, snapshot) => (
                                            ...
                                            >
                                                ...
                                                { renderBlock(childBlock, selectedBlockId === childBlock.id, selectedBlockId === childBlock.id) }
                                            </div>
                                        ) }
                                    </Draggable>
                                ))
